...
def test_single_prediction():
    ...
    try:
        ...
        print("[OK] Low risk application prediction:")
        ...
        print("\n[OK] High risk application prediction:")
        print(f"   Risk Score: {result['risk_score']:.1f}%")
        print(f"   Decision: {result['decision']}")
        print(f"   Risk Category: {result['risk_category']}")
        ...
        print(f"[FAIL] Error making prediction: {str(e)}")
        return False
...
def test_batch_prediction():
    ...
    try:
        ...
        print("[OK] Batch prediction completed successfully")
        ...
        print(f"[FAIL] Error with batch prediction: {str(e)}")
        return False
...
def test_api_wrapper():
    ...
    try:
        ...
        print("[OK] API wrapper test successful")
        ...
        print(f"[FAIL] Error with API wrapper: {str(e)}")
        return False
...
def test_feature_importance():
    ...
    try:
        ...
        if importance_df is not None:
            print("[OK] Feature importance loaded successfully")
            print("\n   Top 10 Important Features:")
            for _, row in importance_df.iterrows():
                print(f"   - {row['feature']}: {row['importance']:.4f}")
            return True
        else:
            print("[WARN] Feature importance not available (this is expected for some model types)")
            return True

    except Exception as e:
        print(f"[FAIL] Error loading feature importance: {str(e)}")
        return False
...
def test_edge_cases():
    ...
    try:
        ...
        print("[OK] Handled incomplete application")
        print(f"   Risk Score: {result['risk_score']:.1f}%")
        ...
        print(f"[FAIL] Error with edge cases: {str(e)}")
        return False
...
def run_all_tests():
    ...
    for test_name, test_func in tests:
        try:
            ...
            print(f"\n[FAIL] {test_name} failed with exception: {str(e)}")
